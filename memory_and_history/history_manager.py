...
class HistoryManager:
    ...
    def initialize_histories(
        ...
        Args:
            ...
            A1_initial: A1's initial position {"labels": [...], "opinion": str, "evidence": str}
            A2_initial: A2's initial position {"labels": [...], "opinion": str, "evidence": str}
    ...
    def build_history_text(
        ...
        if len(history) > 1:  # Skip round 0 (initial)
            ...
            for round_data in history[1:]:  # Start from round 1
                ...
                # Support both single label (legacy) and multi-label list
                if "labels" in response:
                    labels_str = ", ".join(
                        f"{l['entity']}#{l['attribute']}|{l['sentiment']}"
                        for l in response["labels"]
                    )
                    history_text += f"Labels: [{labels_str}]\n"
                elif "label" in response:
                    lbl = response["label"]
                    history_text += f"Labels: [{lbl['entity']}#{lbl['attribute']}|{lbl['sentiment']}]\n"
        else:
